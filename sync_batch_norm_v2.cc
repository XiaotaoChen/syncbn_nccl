 ...
 * \file sync_batch_norm_v2.cc
 * \brief Synchronized BatchNorm modified from BatchNormV1 with nccl communication
 * \author Xiaotao Chen
*/

#include "sync_batch_norm_v2-inl.h"
#include <nnvm/op_attr_types.h>

namespace mxnet {
namespace op {
template<>
Operator *CreateOp<cpu>(SyncBatchNormV2Param param, int dtype) {
  return new SyncBatchNormV2<cpu>(param);
}

// DO_BIND_DISPATCH comes from operator_common.h
Operator *SyncBatchNormV2Prop::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(SyncBatchNormV2Param);

MXNET_REGISTER_OP_PROPERTY(_contrib_SyncBatchNormV2, SyncBatchNormV2Prop)
...
.add_arguments(SyncBatchNormV2Param::__FIELDS__());

NNVM_REGISTER_OP(_contrib_SyncBatchNormV2)
