 ...
 * \file sync_batch_norm_v3.cc
 * \brief Synchronized BatchNorm modified from BatchNormV1 with nccl communication
 * \author Xiaotao Chen
*/

#include "sync_batch_norm_v3-inl.h"
#include <nnvm/op_attr_types.h>

namespace mxnet {
namespace op {
template<>
Operator *CreateOp<cpu>(SyncBatchNormV3Param param, int dtype) {
  return new SyncBatchNormV3<cpu>(param);
}

// DO_BIND_DISPATCH comes from operator_common.h
Operator *SyncBatchNormV3Prop::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(SyncBatchNormV3Param);

MXNET_REGISTER_OP_PROPERTY(_contrib_SyncBatchNormV3, SyncBatchNormV3Prop)
...
.add_arguments(SyncBatchNormV3Param::__FIELDS__());

NNVM_REGISTER_OP(_contrib_SyncBatchNormV3)
